...
#' @param n.subsets a scalar indicating how many samples to draw to find a homogenous
#' starting base group
...
#' output
#' }
forward.search <- function(data, nfact, criteria = c('LD', 'mah'), 
...
{	
	...
	if(is.numeric(nfact)){
		...
		for(i in 1:n.subsets){
			mod <- factanal(data[Samples[ ,i], ], nfact)
		...
		for (LOOP in 1:length(nbaseID)){			
			tmpcor <- cor(basedata)
			basemodels[[LOOP]] <- mlfact(tmpcor, nfact)
			...
			if(any(criteria == 'LD')){				
				for(j in 1:length(nbaseID))
					stat[j] <- mlfact(cor(rbind(basedata, data[nbaseID[j], ])), nfact)$value										
				RANK <- RANK + rank(stat)
			}
			if(any(criteria == 'mah')){	
				stat <- mahalanobis(data[nbaseID, ], colMeans(data[baseID, ]), cov(data[baseID, ]))
				RANK <- RANK + rank(stat)	
			}
			if(any(criteria == 'res')){
				stat <- c()
				for(j in 1:length(nbaseID)){					
					tmp <- obs.resid(rbind(basedata, data[nbaseID[j], ]), nfact)
					stat[j] <- tmp$std_res[nrow(basedata)+1, ] %*% tmp$std_res[nrow(basedata)+1, ]
				}
				RANK <- RANK + rank(stat)
			}
			RANK <- rank(RANK)
		...
		basemodels[[LOOP+1]] <- mlfact(cor(data), nfact)	
		...
		for(i in 1:(length(basemodels)-1)){
			...
			diag(Rhat) <- 1
			RMR[i] <- sqrt(2*sum(((basemodels[[i]]$R - Rhat)^2)/(nfact*(nfact + 1))))
		}
		...
		diag(Rhat) <- 1
		RMR[i+1] <- sqrt(2*sum(((basemodels[[i+1]]$R - Rhat)^2)/(nfact*(nfact + 1))))
